...
/// Render the profile into a generation's directory, before the flip
/// that activates it. A generation with no contributions carries no
/// profile — the rc-side `[ -f ... ]` guard reads that as "nothing to
/// export". The legacy pre-0.22 `env/profile.sh` is removed so it can
/// never shadow the generation-local one for users who sourced it.
pub fn render_env_file(
    ...
    if !any {
        let _ = cap.remove_file(&rel);
        return Ok(path);
    }
    gripsack_fs::atomic_write(&cap, &rel, lines.join("\n").as_bytes())?;
    Ok(path)
}
