...
| Leaf | Class / target | Prerequisite, owner and implementation | State / evidence | Unmet acceptance |
|---|---|---|---|---|
| M0-1.2a | NEXT / M0 | Facts are captured in `commands/eval.rs` before any repo build overlay; operator env selects `facts::glibc_version`'s `ldd`, never repo `PATH`. | **Implemented-unverified at `1c693ef`**: pre-fix real-CLI repo `PATH` shim executed and wrote a marker; post-fix Linux and exact-source native macOS arm64 full e2e **319/319** do not select the fake `glibc-999.0` branch. The direct 12-test/four-model receipt and native CI job `108365100726` bind the observed paths. | R5/M-V7 process/identity proof, other plan NEXT work and release-wide platform qualification remain open; a successful workflow dispatch is not branch protection. |
| M0-1.2b | NEXT / M0 | `gripsack-config::parse_env_as` rejects both GitHub host audiences and four token names alongside `GRIPSACK_*` with E400 and a source span; `http::Policy` reads operator credentials, not the repo build overlay. | **Implemented-unverified at `1c693ef`**: before the change repo `GH_HOST=127.0.0.1` sent a dummy operator token over local HTTP and `grip update` exited 0; terminal `update` and JSON `check` now both report source-labeled E400 before any request. Six-name Rust unit and Linux/native arm64 macOS full e2e **319/319** pass at this exact source. | Global credential/IR proof and protected merge/release gates remain open. |
| M0-1.2c | NEXT / M0 | `FetchContext` owns `BuildProcessEnv`: artifact HTTP captures repo proxy/CA; selected build/verify/git/pixi/plugin children receive explicit env maps. Provisioning, core facts and Deno retain operator env; per-step overrides and dependency-closure PATH win over the base overlay. | **Implemented-unverified at `1c693ef`**: original process-global `set_var` loop removed. Direct committed-source real build shell, structured run PATH, plugin discovery/capabilities/fetch, proxy/NO_PROXY and Deno-wrapper probes pass; full Linux and native macOS arm64 e2e each pass **319/319**, and direct TypeScript/frontend **64/64** plus strict examples pass. | R5/M-V7 process-boundary proof, unreached platform/VM lanes and release-wide gating remain open. |
| M0-1.2d | NEXT / M0 | `http::Policy` matches credentials only on HTTPS; HTTP refuses initially bound non-HTTPS public/enterprise or registry authorization before network, and never forwards auth on redirect. Dummy enterprise fixture uses offline TLS CA/leaf, no production loopback exception. | **Implemented-unverified at `1c693ef`**: sourcebound local TLS enterprise update→cold apply, wrong- and same-host redirect without bearer forwarding, repo-scoped CA/proxy, HTTP zero-request refusal and registry-auth Rust unit pass. The exact-source native arm64 Mac full e2e also passes **319/319**; job log archived as `verification/reports/2026-09-26-m0-env-macos-ci-1c693ef.log`. | No TLS library theorem, Mac-VM/launchd qualification or R5/M-V7 proof-to-effect claim. |
| M0-1.2e | NEXT / M0 | Join real PATH/GH_HOST/HTTP probes, ordinary child env, E400 terminal/JSON, production host selector and `CredentialRouting.tla` operator-audience/no-cleartext transitions with named repo-audience mutant. | **In progress, not an M0 or release closure**: committed-source report `verification/reports/2026-09-26-m0-env-1c693ef.log` at `1c693ef` and fingerprint `9a82d56b48bef69d8a57c831a3efa16638123279150689d277ecdd8a81447825` binds Rust/real CLI **12/12** and full TLC with credential positive/three mutants **4/4**. Exact-source manual CI run [`36227812302`](https://github.com/gripsack-dev/gripsack/actions/runs/36227812302) passed Linux `test` (Rust, TS, e2e, TLC, Verus), native macOS **319/319**, audit, fuzz and docs; local identical-source e2e **319/319**, Deno **64/64**, Verus **72/0** plus seven mutants also passed (local full-gate output attribution separately marked inferred). | Manual dispatch does **not** enforce protected PR status; R5/M-V7 proof-to-effect, all other plan NEXT work and 178-ID handover remain blocking. Checker-source `56b1581` is a later change and needs its own CI. |

The later checker-source CI dispatch
[`36230541824`](https://github.com/gripsack-dev/gripsack/actions/runs/36230541824)
checked out `52943b2` and passed the 24-negative delivery gate,
then **failed required `test`**: the self-update test rewrote and
