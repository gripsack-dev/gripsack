...
| 0052 | A1 workspace contract: v4 history, v5 typed admission and partial graph/command/diagnostic gates | (pending) | Strict v3 modules and read-only v4 workspace retained. Current v5 schema/Rust/TS supports nine output kinds, typed targets/layouts, distinct graph roles, fluent/object commands and source-mapped literal Bash; unavailable effects fail E124 before activation. The adapter rejects role/target/payload/kind/binding substitutions and missing/replaced/extra catalog entries with E131; a missing producer was silently accepted before the catalog guard, then seven adapter regressions passed. Production admission and tests occupy separate cohesive modules. `WORKSPACE-TARGET-001` proves declared OS/arch/ABI/minimum-OS comparison; production-used `WORKSPACE-INDEX-001` binds graph endpoints and package root/producer only when each candidate position names the declaration, with a private bound-index constructor and an attributable alias mutant. This is pointwise, not full schema-to-closure refinement. A1-06's single diagnostic manifest now generates a static frontend membership Record: forged unallocated E999 was structured before the fix and remains a traceback with no JSON diagnostic after it; valid generated codes remain unchanged. Four offline workspaces typecheck and pass CLI admission. The post-diagnostic Linux compose chain passed fresh Rust fmt/clippy/tests, Deno 63/63, real CLI e2e 290/290 and Verus 72/0 with seven policy mutants; TLC was cached. Earlier `2b7daf8` source-bound reports predate the TS/e2e code change and cannot close current A1 rows. IR enum conversion, source-edge completeness, diagnostic classification, protected CI and native Mac evidence remain open | A1-01 implemented_unverified; A1-02/A1-03/A1-06/A1-07/A1-10 in_progress; A1-04/05/08/09/11/12 and plan/0048 NEXT remain open; no release |
