...
| 0052 | A1 workspace contract: v4 history, v5 typed admission and partial graph/command/diagnostic gates | (pending) | Strict v3 modules and read-only v4 workspace retained. Current v5 schema/Rust/TS supports nine output kinds, typed targets/layouts, distinct graph roles, fluent/object commands and source-mapped literal Bash; unavailable effects fail E124 before activation. The graph adapter rejects role/target/payload and target-kind/binding projection substitutions with E131; the broadened-kind regression failed before the guard and passed after. Cohesive declaration/coverage/diagnostic modules replaced a 698-line catch-all. A production-used Verus `TargetRequirement` kernel now decides exact declared OS/arch/ABI and minimum-OS compatibility for recipe producers and package selections, replacing the IR's unproved duplicate comparator; an ABI mutant fails its named assertion. Its typed IR conversion and the schema/name-index graph bridge remain unproved. A1-06 generates 36 Rust and five frontend diagnostic codes from one byte-fresh registry with three negative calibrations. Four offline TypeScript workspaces typecheck and pass real CLI admission. The post-target Linux compose chain passed fresh Rust fmt/clippy/tests, real CLI e2e 289/289 and fresh Verus 68/0 with six policy mutants; TS and TLC images were cached. Prior source-bound `a754e04` reports predate this target/fixture edit; no new source-bound closure claim, workspace realization, protected CI or native Mac execution is asserted | A1-01 implemented_unverified; A1-02/A1-03/A1-06/A1-07/A1-10 in_progress (schema→adapter refinement, pinned Bash/options, diagnostic classification, compact API/proof/CI/Mac evidence); A1-04/05/08/09/11/12 and plan/0048 NEXT remain open; no release |
