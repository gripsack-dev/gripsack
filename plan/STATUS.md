...
| 0052 | A1 workspace contract: v4 history, v5 typed admission and partial graph/command authoring | (pending) | Retained strict v3 modules and read-only v4 workspace; current v5 schema/Rust/TS adds typed execution/ABI/minimum-OS, fixed-prefix environment selection and distinct graph roles. CLI/direct executor APIs reject v4/v5 realization with E124. A1-02 Linux compose passed Rust tests, TS 61/61, e2e 262/262 and Verus 61/0 plus five mutants; model used a CACHED TLC layer. A1-03 adds immutable fluent/object exec and Bash values, canonical env data, literal-body source maps and Rust E130 rejection of forged decoded interpolation. Post-correction Linux compose passed Rust fmt/clippy/tests, e2e 264/264 and fresh Verus 61/0 with five mutants; focused Deno 63/63 and CLI 18/18 passed, final TS/model layers cached. No workspace execution | A1-01 implemented_unverified; A1-02 in_progress (full schema→adapter refinement, source-bound CI/Mac); A1-03 in_progress (strict Bash options/byte pin, static interpolation guard, normalization proof, source-bound CI); A1-04–12 and plan/0048 NEXT remain open; no release |
