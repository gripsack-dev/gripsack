...
| 0052 | A1 workspace contract: v4 history, v5 typed admission and partial graph/command/diagnostic gates | (pending) | Strict v3 modules and read-only v4 workspace retained. Current v5 schema/Rust/TS supports nine output kinds, typed targets/layouts, distinct graph roles, fluent/object commands and source-mapped literal Bash; unavailable effects fail E124 before activation. The adapter rejects role/target/payload/kind/binding substitutions and missing/replaced/extra catalog entries with E131; a missing producer was silently accepted before the catalog guard, then seven adapter regressions passed. Production admission and tests live in separate cohesive modules. `WORKSPACE-TARGET-001` proves exact OS/arch/ABI and minimum-OS comparison. A new production-used `WORKSPACE-INDEX-001` kernel binds each projected edge endpoint and package root/producer only when the borrowed catalog name at its candidate index exactly matches; distinct valid names cannot alias an index. Its private bound-index constructor and an attributable alias mutant prevent a forged or unchecked index reaching closure through this adapter. This is a pointwise theorem: serde decoding, source-edge completeness, IR enum mapping and the full schema-to-closure refinement remain unproved. A1-06 retains its single byte-fresh diagnostic registry; four offline TypeScript workspaces pass typecheck and CLI admission. The post-index Linux compose chain passed fresh Rust fmt/clippy/tests, real CLI e2e 289/289 and Verus 72/0 with seven mutants; TS and TLC images were cached. Earlier `0d7801e` source-bound reports predate this policy/code change. No workspace realization, protected CI or native Mac execution is claimed | A1-01 implemented_unverified; A1-02/A1-03/A1-06/A1-07/A1-10 in_progress (schema→adapter refinement, pinned Bash/options, diagnostic classification, compact API/proof/CI/Mac evidence); A1-04/05/08/09/11/12 and plan/0048 NEXT remain open; no release |
