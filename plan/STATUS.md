...
| 0052 | A1 workspace contract: v4 history, v5 typed admission and partial graph/command/diagnostic gates | (pending) | Strict v3 modules and read-only v4 workspace retained. Current v5 schema/Rust/TS supports nine output kinds, typed targets/layouts, distinct graph roles, fluent/object commands and source-mapped literal Bash; unavailable effects fail E124 before activation. The graph adapter rejects role/target/payload and target-kind/binding projection substitutions with E131; the broadened-kind regression failed before the guard and passed after. Cohesive declaration/coverage/diagnostic modules replaced a 698-line catch-all. The production-used `TargetRequirement` kernel proves exact declared OS/arch/ABI and minimum-OS compatibility for recipe producers and package selections, replacing the IR's duplicate comparator; the ABI mutant fails its named assertion. IR enum conversion and the schema/name-index graph bridge remain unproved. A1-06 generates 36 Rust and five frontend diagnostic codes from one byte-fresh registry with three negative calibrations. Four offline TypeScript workspaces typecheck and pass real CLI admission. The post-target Linux compose chain passed fresh Rust fmt/clippy/tests, real CLI e2e 289/289 and fresh Verus 68/0 with six policy mutants; TS and TLC images were cached. Source-bound `2699d79` local reports record 56 schema/frontend/CLI cases, 9 example checks, 3 E124 owners, 8 combined adapter/real-target cases, one architecture self-check, 84 diagnostic checks and 68 policy obligations. Earlier `a754e04` receipts remain historical. No workspace realization, protected CI or native Mac execution is claimed | A1-01 implemented_unverified; A1-02/A1-03/A1-06/A1-07/A1-10 in_progress (schema→adapter refinement, pinned Bash/options, diagnostic classification, compact API/proof/CI/Mac evidence); A1-04/05/08/09/11/12 and plan/0048 NEXT remain open; no release |
