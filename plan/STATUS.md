...
| 0052 | A1 workspace contract: v4 history, v5 typed admission and partial graph/command/diagnostic gates | (pending) | Strict v3 modules and read-only v4 workspace retained. Current v5 schema/Rust/TS supports nine output kinds, typed targets/layouts, graph roles, fluent/object commands and source-mapped literal Bash; unavailable effects fail E124 before activation. The graph adapter rejects reference substitution and missing/replaced/extra catalog entries with E131; a failing-before source-site regression now guards both command and producer attribution, and source coverage gates index projection/closure. The pointwise production-used `WORKSPACE-INDEX-001` binds each candidate to the exact declared name, while `WORKSPACE-TARGET-001` proves declared target comparison; neither establishes full serde/source-edge/IR-enum refinement. A1-06's single manifest generates 36 Rust codes, five frontend constants and a static frontend membership Record; a failing-before unallocated E999 now stays a traceback with empty JSON diagnostics. Four offline TypeScript workspaces typecheck and pass CLI admission. At code SHA `ec11c7c`, the source-bound local diagnostic report records generator freshness, three registry mutants, Deno 63/63 and 18 real CLI cases (85 checks); older reports predate the new source guard. The post-diagnostic Linux compose chain passed fresh Rust fmt/clippy/tests, Deno 63/63, real CLI e2e 290/290 and fresh Verus 72/0 with seven policy mutants; TLC was cached. Post-guard Rust fmt/clippy/full tests, direct span regression and real CLI workspace contract 23/23 passed, but new source-bound proof/CLI receipts are not yet recorded. Diagnostic classification, graph completeness, protected CI and native Mac remain open; no workspace execution or release is claimed | A1-01 implemented_unverified; A1-02/A1-03/A1-06/A1-07/A1-10 in_progress; A1-04/05/08/09/11/12 and plan/0048 NEXT remain open |
