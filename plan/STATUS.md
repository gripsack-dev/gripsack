...
| 0052 | A1 workspace contract: v4 history, v5 typed admission and partial graph/command/diagnostic gates | (pending) | Strict v3 modules and read-only v4 workspace retained; current v5 schema/Rust/TS adds typed targets/layouts, distinct graph roles and E124 before workspace effects. A1-02 base Linux compose passed Rust, TS 61/61, e2e 262/262, fresh Verus 61/0 with five mutants (TLC cached); later role-cardinality admission rejects omitted build/runtime edges with E131, not a full name-index proof. A1-03 fluent/object commands and literal source-mapped Bash passed Linux compose e2e 264/264. A1-06 adds structured frontend/core diagnostic parity in `grip check --json`, repo-capability-bounded snippets (1 MiB) and an output-specific E124 gate for B2/E2/E3/E1 and historical v4 A5 migration. A1-07 expands calibrated architecture checks to every Cargo workspace member including fuzz; one offline embedded-SDK fixture now goldens all nine v5 kinds with a semantic Bash-env mutant and a real admission-only CLI case. Latest Linux compose passed Rust fmt/clippy/tests, fresh Deno 62/62 and e2e 279/279; TLC and Verus image layers CACHED. Source-stamped local reports at ff73b62 bind 35 focused golden/workspace/diagnostic cases and fresh 61 policy obligations plus the named graph mutant; earlier d6d29d7 reports are history, neither run proves the name-index bridge or CI/Mac. No workspace execution | A1-01 implemented_unverified; A1-02/A1-03/A1-06/A1-07 in_progress (schema→adapter refinement, pinned Bash/options, remaining capability owners, registry/proof/CI/Mac evidence and compact API cutover); A1-04/05/08–12 and plan/0048 NEXT remain open; no release |
