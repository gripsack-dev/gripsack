...
| 0052 | A1 workspace contract: v4 history, v5 typed admission and partial graph/command/diagnostic gates | (pending) | Strict v3 modules and read-only v4 workspace retained. Current v5 schema/Rust/TS supports typed targets/layouts, nine output kinds, distinct graph roles, fluent/object commands and source-mapped literal Bash; unavailable effects fail E124 before activation. The production graph adapter rejects role/target/selector/exported-command substitutions before using the proved role/closure kernels, but its schema/name-index/binding correspondence is unproved. A1-06 generates 36 Rust and five frontend diagnostic codes from one allocation registry, with generated-byte freshness and three named negative calibrations. Four graduated offline TypeScript workspaces are strictly compiled and admitted by the real CLI without effects. At code SHA `6c759a5`, SHA-256 checked local reports bind 56 schema/frontend/CLI cases, 9 example checks, 3 E124 owners, 5 adapter cases, one architecture self-check, 84 diagnostic registry/frontend/CLI checks, and 61 Verus policy obligations with the graph-validation mutant; the three registry mutants are a subset of the 84. Linux worktree gates passed fresh Rust fmt/clippy/tests, Deno 63/63, real CLI e2e 289/289 and fresh Verus 61/0; TLC was cached. No workspace realization, protected CI or native Mac execution is claimed | A1-01 implemented_unverified; A1-02/A1-03/A1-06/A1-07/A1-10 in_progress (schema→adapter refinement, pinned Bash/options, diagnostic classification, compact API/proof/CI/Mac evidence); A1-04/05/08/09/11/12 and plan/0048 NEXT remain open; no release |
