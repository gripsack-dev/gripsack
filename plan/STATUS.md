...
| 0052 | A1 workspace/IR-v4 contract, read-only admission and partial graph validation | (pending) | v4 schema + Rust typed admission + TS workspace emitter integrated; strict v3 reader retained, project entry needs neither hosts/<name>.ts nor env.toml; `grip check` lists named outputs and plan/apply/update/adopt reject E124 before effects. A1-01: nine typed kinds, mandatory spans, provider-backed package and golden corpus. A1-02 static graph packet: role-typed Rust/TS projection, source-aware ref/kind/selector/cycle/target/layout admission; real CLI focused 11/11 and final Linux compose chain full e2e 257/257 (TLC/Verus final layers cached; existing policy Verus 56 previously executed, not A1 proof). No workspace execution | A1-01 implemented_unverified; A1-02 in_progress (production graph policy adapter, formal correspondence/mutant, full ABI/prefix model, source-bound CI still open). A1-03–12, H0/A0/B0 closures and plan/0048 NEXT remain open; no release |
