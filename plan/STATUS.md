...
| 0052 | A1 workspace contract: v4 history, v5 typed admission and partial graph/command/diagnostic gates | (pending) | Strict v3 modules and read-only v4 workspace retained; current v5 schema/Rust/TS adds typed targets/layouts, distinct graph roles and E124 before workspace effects. A1-02 production adapter streams decoded role/target, artifact selector and exported package-command names against projected edges: missing/reordered/reclassified and same-role target/payload substitutions fail E131 without added successful-path allocations; a selector substitution failed before and passed after the guard. This is runtime correspondence, not schema/name-index, expected-kind/binding or provenance proof. A1-03 fluent/object commands and literal source-mapped Bash remain unexecuted declarations without a pinned interpreter/options cutover. A1-06 retains structured terminal/JSON parity, bounded repo-capability snippets and first-output E124 owners B2/E2/E3/E1/v4 A5 plus newly exercised image B4, environment A2-P and check A2/E1; user/engine exception and import-syntax regressions preserve tracebacks. A1-07 has calibrated architecture checks over every Cargo member, an offline all-nine-kind golden, real admission-only CLI case, semantic Bash-env mutant and v5 schema/parser unknown-field negatives under every kind. A1-10 adds four actual offline workspaces: strict TypeScript 7 compiles their source; real CLI admits all four and a sandboxed Deno eval switches one package's producer on injected arch (5/5 focused cases). New `examples/` is tracked by SOURCE_ROOTS. Latest Linux compose passed fresh Rust fmt/clippy/tests, fresh real CLI e2e 289/289 and a cached TS/TLC/Verus layer; the separate fresh TypeScript 63/63 plus four-example typecheck ran before that chain. The source-stamped local reports at 530ae7d bind 51 corpus, three E124 owner, five adapter, one architecture and 61 policy obligations to the previous source, not this changed example tree or protected CI/Mac. No workspace execution | A1-01 implemented_unverified; A1-02/A1-03/A1-06/A1-07/A1-10 in_progress (schema→adapter refinement, pinned Bash/options, remaining capability owners, compact API/examples proof and CI/Mac evidence); A1-04/05/08/09/11/12 and plan/0048 NEXT remain open; no release |
