...
| 0052 | A1 workspace contract: v4 history, v5 typed admission and partial graph/command/diagnostic gates | (pending) | Strict v3 modules and read-only v4 workspace retained. Current v5 schema/Rust/TS supports nine output kinds, typed targets/layouts, graph roles, fluent/object commands and source-mapped literal Bash; unavailable effects fail E124 before activation. The graph adapter rejects reference substitution and missing/replaced/extra catalog entries with E131; the pointwise production-used `WORKSPACE-INDEX-001` binds each candidate index to the exact declared name before closure, while `WORKSPACE-TARGET-001` proves declared target comparison. Their guards and the named alias/ABI mutants do not prove full serde/source-edge/IR-enum refinement. A1-06's single manifest now generates all 36 Rust codes, five frontend constants and a static frontend membership Record; an unallocated E999 surfaced as a structured error before the fix and remains a traceback with empty JSON diagnostics afterward. Four offline TypeScript workspaces typecheck and pass CLI admission. The post-diagnostic Linux compose chain passed fresh Rust fmt/clippy/tests, Deno 63/63, real CLI e2e 290/290 and fresh Verus 72/0 with seven policy mutants; TLC was cached. At code SHA `ec11c7c`, a source-bound local report records generator freshness, three named registry mutants, fresh Deno 63/63 and 18 real CLI diagnostics (85 checks). Previous `2b7daf8` graph/proof receipts and other row reports precede this TS/e2e edit. Diagnostic classification, graph completeness, protected CI and native Mac remain open; no workspace execution or release is claimed | A1-01 implemented_unverified; A1-02/A1-03/A1-06/A1-07/A1-10 in_progress; A1-04/05/08/09/11/12 and plan/0048 NEXT remain open |
