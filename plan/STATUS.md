...
| 0049 | Handover bundle import (edition 5) — H0 reconciliation + A0 bottle selection | (pending) | checksums verified, 178 delivery IDs registered in verification/delivery.json with lane/case inventories for H0/A0/B0/E0/A1; unmodified baseline gates recorded from a pristine worktree at 176eaec; A0-01 replaces lexical bottle_key with a pure policy over injected HostPlatform facts (codename chronology table, structural tag grammar, last-resort `all`, refusal reasons) with synthetic-fact unit tests for every acceptance case | B0/E0 qualification records; A1 compact API + delivery-closure checker (scripts/check_delivery.py) with negative calibration; plan/0048 NEXT remains binding for any public release; native macOS lanes blocked (no Mac hardware) and recorded as such |
| 0050 | E0 qualification — task/schedule scope, vocabulary, proof targets | (pending) | E0-01 record: current-source→A1/A2-P mapping, eight scenario families with owners, frozen result/exit vocabulary (failed/blocked/cancelled/skipped-busy/indeterminate), v1 budgets/boundaries, E1/E3 production proof targets; E0-02: systemd 255 user-manager lane verified by real fixture journey (install→OnCalendar trigger→zero-residue removal), launchd lane blocked (no Mac) and recorded open | E1 gated on A1/A2/A2-P; TLAPS lane is new infrastructure to stand up when E1/E3 proof work lands |
| 0051 | B0 qualification — BuildKit harness + integration inventory | (pending) | B0-04 record: LLB op/field matrix, bridge protocol states/fences, proof families/mappings, emitted-LLB witness mechanism, scheduler migration plan; B0-01 VERIFIED on linux/amd64: pinned disposable harness (Go bridge over moby/buildkit v0.33.0, built in digest-pinned golang container) exercising shared-dep reuse, cancellation, bounded failure attribution, static executable from digest-pinned gcc (network=none) retained after worker+cache destruction, OCI export reproduced from two fresh workers + independently verified + run by the docker engine, include-pattern/credential/host-path policy probes; B0-03 Linux measurements + zero-builder baseline recorded | B0-02 Mac VM lane blocked (no Mac); B1 gated on A1; B0-03 VM-lane budgets at B1; evidence binds on PR landing |
