...
| 0052 | A1 workspace contract: v4 history, v5 typed admission and partial graph/command/diagnostic gates | (pending) | Strict v3 modules and read-only v4 workspace retained. Current v5 schema/Rust/TS supports nine output kinds, typed targets/layouts, distinct graph roles, fluent/object commands and source-mapped literal Bash; unavailable effects fail E124 before activation. The adapter rejects role/target/payload/kind/binding substitutions and missing/replaced/extra catalog entries with E131; a missing producer was silently accepted before the catalog guard, then seven adapter regressions passed. Production admission and tests live in separate cohesive modules. `WORKSPACE-TARGET-001` proves declared OS/arch/ABI and minimum-OS comparison. Production-used `WORKSPACE-INDEX-001` binds projected edge endpoints and package root/producer only when the borrowed catalog name at the candidate index equals the declaration. The bound index has a private constructor; Verus proves pointwise exact binding and distinct-name non-aliasing, and an in-range alias mutant fails the named assertion. This does **not** prove serde decoding, source-edge completeness, IR enum mapping or full schema-to-closure refinement. A1-06 retains its single diagnostic allocation registry; four offline TypeScript workspaces pass typecheck and CLI admission. The post-index Linux compose chain passed fresh Rust fmt/clippy/tests, real CLI e2e 289/289 and Verus 72/0 with seven mutants; TS and TLC images were cached. At code SHA `2b7daf8`, local source-bound reports record seven adapter cases, one name-binding case, two real CLI target cases and 72 proved policy obligations; other A1 rows' receipts bind earlier code. No workspace realization, protected CI or native Mac execution is claimed | A1-01 implemented_unverified; A1-02/A1-03/A1-06/A1-07/A1-10 in_progress (schema→adapter refinement, pinned Bash/options, diagnostic classification, compact API/proof/CI/Mac evidence); A1-04/05/08/09/11/12 and plan/0048 NEXT remain open; no release |
