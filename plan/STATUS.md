...
| 0048 | Unified review handoff with binding release and quality gates | (pending) | planning only: retains 18 original findings, dispositions all 16 external findings and integrates R1–R8; owner follow-up makes §9 delivery/deferral rules and §10 readable domain-typed/cohesive-module code mandatory; no production changes or newly run product verification | NEXT / M0–M2: boundary fixes, typed policies/refinement, hooks, release/assurance/install/public contracts and all M-V1–M-V7 (TLAPS pilot + generalized recovery, Layer-2/transition proofs, Loom/parser/GC/protocol campaigns); explicitly authorized M3/M4 product work remains required/open; VM/platform evidence is claim-gated; only M-V8 Lean/Aeneas is exploratory; required failures block release pending owner decision | unilateral deferral/acceptance weakening; proptest substituted for a required proof; partial scaffold called done; token/diff efficiency over readable code; naked semantic IDs/units and tuple policy flags; deferred necessary module splits; duplicate policy/refinement implementations; fake filesystem/containment/verification claims; unapproved publication/archival/yanking |
| 0049 | Handover bundle import (edition 5) — H0 reconciliation + A0 bottle selection | (pending) | checksums verified, 178 delivery IDs registered in verification/delivery.json with lane/case inventories for H0/A0/B0/E0/A1; unmodified baseline gates recorded from a pristine worktree at 176eaec (logs archived byte-exact in verification/reports/); A0-01 replaces lexical bottle_key with a pure policy over injected HostPlatform facts (codename chronology table, structural tag grammar, last-resort `all`, refusal reasons) with synthetic-fact unit tests for every acceptance case; **H0/A0 closure claims retracted 2026-09-24** — evidence was Markdown-only (H0-01/A0-01 implemented_unverified, H0-02 in_progress with 149 rows still null-inventory; prior claims kept as historical_claims with invalid_evidence) | B0/E0 qualification records; A1 compact API + delivery-closure checker (scripts/check_delivery.py) with negative calibration; plan/0048 NEXT remains binding for any public release; native macOS lanes blocked (no Mac hardware) and recorded as such |
...
| 0052 | A1 workspace contract: v4 history, v5 typed admission and partial graph/command/diagnostic gates | (pending) | Strict v3 modules and read-only v4 workspace retained; current v5 schema/Rust/TS adds typed targets/layouts, distinct graph roles and E124 before workspace effects. A1-02 production adapter streams decoded role/target, artifact selector and exported package-command names against projected edges: missing/reordered/reclassified and same-role target/payload substitutions fail E131 without added successful-path allocations; a selector substitution failed before and passed after the guard. This is runtime correspondence, not schema/name-index, expected-kind/binding or provenance proof. A1-03 fluent/object commands and literal source-mapped Bash remain unexecuted declarations without a pinned interpreter/options cutover. A1-06 retains structured terminal/JSON parity, bounded repo-capability snippets and first-output E124 owners B2/E2/E3/E1/v4 A5 plus image B4, environment A2-P and check A2/E1; user/engine exception and import-syntax regressions preserve tracebacks. A1-07 has calibrated architecture checks over every Cargo member, an offline all-nine-kind golden, real admission-only CLI case, semantic Bash-env mutant and v5 schema/parser unknown-field negatives under every kind. A1-10 adds four actual offline workspaces: strict TypeScript 7 compiles the four source files, real CLI admits each, and sandboxed Deno switches one package's producer on injected arch (5/5 focused cases); `examples/` participates in SOURCE_ROOTS. Latest Linux compose passed fresh Rust fmt/clippy/tests, fresh real CLI e2e 289/289 and cached TS/TLC/Verus images; a dedicated fresh Deno 63/63 plus strict four-example typecheck ran separately. Source-stamped local reports at 8583a46 bind 56 schema/frontend/CLI cases, four example typechecks plus five named example cases, three E124 owners, five adapter cases, one architecture self-check and fresh 61 policy obligations to this code; previous 530ae7d receipts remain history. This is not protected CI/Mac or an A1-10 normalization/adapter proof. No workspace execution | A1-01 implemented_unverified; A1-02/A1-03/A1-06/A1-07/A1-10 in_progress (schema→adapter refinement, pinned Bash/options, remaining capability owners, compact API/proof and CI/Mac evidence); A1-04/05/08/09/11/12 and plan/0048 NEXT remain open; no release |
