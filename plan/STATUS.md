...
| 0040 | Post-0.35 project sweep | review | boundary review + 13 isolated CLI probes; P1s approved and implemented by 0041 | P2 acquisition/process bounds, causal tracing, executable contract docs, reproducible toolchains/update policy | broad rewrites without a concrete consumer |
| 0041 | P1 contract fidelity and persistence | core/TS 0.36.0 | fail-closed GC; one prepared view; ordering-only needs; output contracts; IR v3 removes inert retries; concrete preview; permission receipts and exact rollback; exhaustive recorded-cut matrix + ordering calibration; cohesive module/test splits | 0040 P2 backlog remains explicit on the roadmap | global step VM and partial retry engine |
