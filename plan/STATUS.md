...
| 0052 | A1 workspace contract: v4 history, v5 typed admission and partial graph/command/diagnostic gates | (pending) | Strict v3 modules and read-only v4 workspace retained. Current v5 schema/Rust/TS supports nine output kinds, typed targets/layouts, distinct graph roles, fluent/object commands and source-mapped literal Bash; unavailable effects fail E124 before activation. The graph adapter rejects role/target/payload/kind/binding substitutions with E131. A new catalog guard also rejects missing, substituted and extra name mappings before the verified role/closure kernels; a missing producer was silently accepted before the fix, and all seven adapter regressions pass after it. Production policy/coverage/diagnostics and adapter tests have separate cohesive modules. The production-used `TargetRequirement` kernel proves exact OS/arch/ABI and minimum-OS compatibility; the ABI mutant fails its named assertion. IR enum conversion and the schema/name-index bridge remain unproved. A1-06 generates 36 Rust and five frontend diagnostic codes from one byte-fresh registry with three negative calibrations. Four offline TypeScript workspaces typecheck and pass CLI admission. The post-catalog Linux compose chain passed fresh Rust fmt/clippy/tests, real CLI e2e 289/289 and fresh Verus 68/0 with six policy mutants; TS and TLC images were cached. Earlier source-bound `2699d79` reports predate this Rust adapter edit; no new source-bound closure claim, workspace realization, protected CI or native Mac execution is asserted | A1-01 implemented_unverified; A1-02/A1-03/A1-06/A1-07/A1-10 in_progress (schema→adapter refinement, pinned Bash/options, diagnostic classification, compact API/proof/CI/Mac evidence); A1-04/05/08/09/11/12 and plan/0048 NEXT remain open; no release |
