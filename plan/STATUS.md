...
| 0052 | A1 workspace contract: v4 history, v5 typed admission and partial graph/command/diagnostic/file gates | (pending) | Strict v3 modules and read-only v4 workspace retained. Current v5 schema/Rust/TS supports nine output kinds, typed targets/layouts, graph roles, fluent/object commands and source-mapped literal Bash; unavailable effects fail E124 before activation. The graph adapter rejects reference substitution and missing/replaced/extra catalog entries with E131; a failing-before source-site regression guards both command and producer attribution, and source coverage gates index projection/closure. The pointwise production-used `WORKSPACE-INDEX-001` binds each candidate to the exact declared name, while `WORKSPACE-TARGET-001` proves declared target comparison; neither establishes full serde/source-edge/IR-enum refinement. A1-06's manifest generates 36 Rust codes, five frontend constants and a static membership Record; unallocated E999 stays a traceback, malformed nested command spans render labeled E129s (no snippet read, E124 never reached), and the E124 owner matrix covers all nine kinds with first-declared precedence. A1-03's `runBash` interpreter is compile-time `WorkspacePackageCommand` (runtime E128 retained for forged objects); resolved bytes/options still await A1-05. A1-11's file contract now composes origin/content/destination with only literal content omitting a source, and a failing-before E102 guard rejects relative/bare-~/dot-dot/trailing destination paths on both authoring and decoded-IR surfaces; the tree origin and rendered-result binding remain unimplemented. Full Docker gates passed at the latest source: Rust fmt/clippy/tests, Deno 64/64, real CLI e2e 297/297. Source-bound receipts: `17513d2` binds destination escapes (81 checks; Deno layer cache-hit at identical SOURCE_ROOTS), `90e68ac` binds span labels/owner matrix/typed interpreter (74 checks), `17ed1f0` binds the graph source-site guard (31 checks plus fresh Verus 72/0 with seven mutants). Diagnostic classification, graph completeness, tree origin, protected CI and native Mac remain open; no workspace execution or release is claimed | A1-01 implemented_unverified; A1-02/A1-03/A1-06/A1-07/A1-10/A1-11 in_progress; A1-04/05/08/09/12 and plan/0048 NEXT remain open |
