...
| 0052 | A1 workspace contract: v4 history, v5 typed admission and partial production graph proof | (pending) | Retained strict v3 modules and historical read-only v4 workspace; current v5 schema/Rust/TS writer adds explicit recipe execution, bounded ABI/minimum-OS, fixed-prefix environment selection and distinct ordering/build/runtime/check edges. `grip check` inspects named outputs without effects; CLI and direct executor APIs reject v4/v5 plan/apply/update with E124. Focused Linux CLI 16/16, Rust IR 67, Deno 61, golden 2, production role/closure Verus 61/0 and five mutants observed. Full post-edit Linux compose: fresh test, TS 61/61, e2e 262/262 and verify 61/0 plus five mutants; model passed with CACHED TLC layer. No workspace execution | A1-01 implemented_unverified; A1-02 in_progress (full schema→adapter refinement and source-bound CI/Mac evidence remain open); A1-03–12 and plan/0048 NEXT remain open; no release |
