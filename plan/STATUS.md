...
| 0052 | A1 workspace/IR-v4 contract and read-only admission | (pending) | v4 schema + Rust typed admission + TS workspace emitter integrated; the core still reads strict v3, new workspace needs neither hosts/<name>.ts nor env.toml, `grip check` lists named outputs, `plan/apply/update/adopt` reject E124 before effects; nine output kinds, mandatory spans, provider-backed package without synthetic recipe, v4 golden fixture (focused real e2e 5/5, golden 2/2). This is A1-01 in progress/implemented-unverified, NOT A1 closure, and no recipe/task/schedule executor is claimed | Full A1-02–12 model/caller/API migration, formal Verus proof and calibrated runtime/native lanes remain open; A1-07 checker/architecture gate partial; H0/A0/B0 closures still open, plan/0048 NEXT binds releases |
