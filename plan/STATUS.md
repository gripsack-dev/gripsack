...
| 0052 | A1 workspace contract: v4 history, v5 typed admission and partial graph/command/diagnostic gates | (pending) | Strict v3 modules and read-only v4 workspace retained; current v5 schema/Rust/TS adds typed targets/layouts, distinct graph roles and E124 before workspace effects. A1-02 production adapter streams decoded role/target, artifact selector and exported package-command names against projected edges: missing/reordered/reclassified and same-role target/payload substitutions fail E131 without added successful-path allocations; a selector substitution failed before and passed after the guard. This is runtime correspondence, not schema/name-index, expected-kind/binding or provenance proof. A1-03 fluent/object commands and literal source-mapped Bash remain unexecuted declarations without a pinned interpreter/options cutover. A1-06 retains terminal/JSON parity, bounded snippets and first-output E124 owners B2/E2/E3/E1/v4 A5/B4/A2-P/A2-E1; the shared `schema/diagnostics.json` now generates all 36 Rust and five frontend codes with a freshness gate and three attributable negative calibrations. User/engine exceptions preserve tracebacks. A1-07 has calibrated architecture checks over every Cargo member, a nine-kind golden, semantic Bash-env mutant and v5 schema/parser unknown-field negatives under every kind. A1-10 adds four actual offline workspaces strictly compiled by TypeScript 7 and admitted by the real CLI; injected facts switch one package's producer, and `examples/` participates in SOURCE_ROOTS. Latest Linux compose passed fresh Rust fmt/clippy/tests, Deno 63/63 plus strict example typecheck, real CLI e2e 289/289 and fresh Verus 61/0 with five policy mutants; TLC cached. Source-stamped local reports at 8583a46 bind 56 schema/frontend/CLI cases, four example typechecks plus five example cases, three E124 owners, five adapter cases, one architecture self-check and 61 policy obligations to the previous code, not this registry cutover or protected CI/Mac. No workspace execution | A1-01 implemented_unverified; A1-02/A1-03/A1-06/A1-07/A1-10 in_progress (schema→adapter refinement, pinned Bash/options, remaining capability owners, diagnostic classification and compact API/proof/CI/Mac evidence); A1-04/05/08/09/11/12 and plan/0048 NEXT remain open; no release |
