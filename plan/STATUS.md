...
| 0052 | A1 workspace contract: v4 history, v5 typed admission and partial graph/command/diagnostic gates | (pending) | Strict v3 modules and read-only v4 workspace retained. Current v5 schema/Rust/TS supports typed targets/layouts, nine output kinds, distinct graph roles, fluent/object commands and source-mapped literal Bash; unavailable effects fail E124 before activation. The production graph adapter rejects role/target/selector/exported-command and target-kind/binding projection substitutions before using proved role/closure kernels. The broadened-kind case **failed before** the source-rooted guard; both kind and binding substitutions **passed after** with E131. Cohesive declaration/coverage/diagnostic modules replaced a 698-line catch-all. Schema/name-index/binding correspondence remains unproved. A1-06 generates 36 Rust and five frontend diagnostic codes from one allocation registry with byte-freshness and three negative calibrations. Four offline TypeScript workspaces are strictly compiled and admitted by the real CLI without effects. The post-split Linux compose chain passed fresh Rust fmt/clippy/tests, real CLI e2e 289/289 and fresh Verus 61/0; TS and TLC images were cached. Source-bound local `a754e04` reports record 56 schema/frontend/CLI cases, 9 example checks, 3 E124 owners, 6 adapter cases, one architecture self-check, 84 diagnostic checks and 61 policy obligations. Prior `6c759a5` receipts are historical; no workspace realization, protected CI or native Mac execution is claimed | A1-01 implemented_unverified; A1-02/A1-03/A1-06/A1-07/A1-10 in_progress (schema→adapter refinement, pinned Bash/options, diagnostic classification, compact API/proof/CI/Mac evidence); A1-04/05/08/09/11/12 and plan/0048 NEXT remain open; no release |
