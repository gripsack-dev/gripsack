...
| 0052 | A1 workspace contract: v4 history, v5 typed admission and partial graph/command/diagnostic gates | (pending) | Strict v3 modules and read-only v4 workspace retained; current v5 schema/Rust/TS adds typed targets/layouts, distinct graph roles and E124 before workspace effects. A1-02 production adapter now streams decoded `(role,target)` refs against projected edges: omitted, reordered, reclassified and same-role substituted targets fail E131; this is not a schema/name-index or payload-identity proof. A1-03 fluent/object commands and literal source-mapped Bash remain unexecuted declarations without a pinned interpreter/options cutover. A1-06 adds structured frontend/core parity in `grip check --json`, bounded repo-capability snippets and output-specific E124 for B2/E2/E3/E1/v4 A5; real user/engine exception and import-syntax regressions preserve traceback rather than minting E130. A1-07 extends calibrated architecture checks to every Cargo member and adds an offline all-nine-kind golden, real admission-only CLI case, semantic Bash-env mutant and v5 schema/parser unknown-field negative under every kind. Latest Linux compose passed Rust fmt/clippy/tests, fresh Deno 63/63, real CLI e2e 281/281 and fresh Verus 61/0 with five mutants; TLC cached. Source-stamped local reports at 123f3bb bind a 48-case v3/v4/v5+frontend corpus, five policy adapter cases, one architecture self-check and fresh 61 policy obligations plus the named graph mutant to this code; earlier ff73b62 reports are history. Neither run proves schema/name-index refinement, CI/Mac or release. No workspace execution | A1-01 implemented_unverified; A1-02/A1-03/A1-06/A1-07 in_progress (schema→adapter refinement, pinned Bash/options, remaining capability owners, registry/proof/CI/Mac evidence and compact API cutover); A1-04/05/08–12 and plan/0048 NEXT remain open; no release |
