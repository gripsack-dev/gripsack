...
| 0052 | A1 workspace/IR-v4 contract cutover design | (pending) | design-only record: live v3 strictly rejects unknown structural fields despite older tolerance prose; A1 is a breaking v4 change, with a versioned v3 reader, explicit state-inventory/migration gate, typed workspace/recipe/task/schedule/file/command model and separate portable pins vs pure mutation locks; provenance required and initial native schedules daily/weekly local only; no schema, emitter, Rust model, runtime or A1 proofs delivered by this record | Implement all schema/Rust/TypeScript sides with caller/golden migration and production-connected Verus evidence; A1-07 ledger checker is partial; H0/A0/B0 closures remain open, plan/0048 NEXT gates bind releases |
