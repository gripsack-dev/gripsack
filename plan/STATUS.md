...
| 0052 | A1 workspace contract: v4 history, v5 typed admission and partial graph/command/diagnostic gates | (pending) | Strict v3 modules and read-only v4 workspace retained. Current v5 schema/Rust/TS supports nine output kinds, typed targets/layouts, distinct graph roles, fluent/object commands and source-mapped literal Bash; unavailable effects fail E124 before activation. The graph adapter rejects role/target/payload/kind/binding substitutions with E131. Its new catalog guard rejects missing, substituted and extra name mappings before the verified role/closure kernels; a missing producer was silently accepted before the fix and all seven focused adapter regressions pass after it. Production admission and its regression tests now occupy separate cohesive modules. The production-used `TargetRequirement` kernel proves OS/arch/ABI and minimum-OS compatibility; the ABI mutant fails its named assertion. IR enum conversion and schema/name-index correspondence remain unproved. A1-06 has a single byte-fresh diagnostic allocation registry; four offline workspaces typecheck and pass CLI admission. The post-catalog Linux compose chain passed fresh Rust fmt/clippy/tests, real CLI e2e 289/289 and fresh Verus 68/0 with six policy mutants; TS and TLC layers were cached. At code SHA `0d7801e`, local source-bound reports record seven adapter cases plus two real CLI target cases and 68 proved policy obligations; other rows' `2699d79` receipts are historical for this changed Rust adapter. No workspace realization, protected CI or native Mac execution is claimed | A1-01 implemented_unverified; A1-02/A1-03/A1-06/A1-07/A1-10 in_progress (schema→adapter refinement, pinned Bash/options, diagnostic classification, compact API/proof/CI/Mac evidence); A1-04/05/08/09/11/12 and plan/0048 NEXT remain open; no release |
